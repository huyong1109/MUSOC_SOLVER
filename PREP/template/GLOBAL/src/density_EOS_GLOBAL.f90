...
SUBROUTINE EOS_COMPUTATION_STABILITY1(T,S,ZZ,RHO,FL_EOS_OP,FL_LEV_OP,EOS_FILE)
 ...
 REAL(8),ALLOCATABLE ::Mcrho(:,:,:),McTP(:,:,:),McS(:,:,:), &
                       McTPTP(:,:,:),McSTP(:,:,:)
...
!                           potential_temperature_T, potential_temperature_TT
 ...
 T=temperature(TP,S,P)
!CALL density_T_S_TT(T,S,P,crho,cTP,cS,cTPTP)
...
SUBROUTINE density_T_S_TT_TS(T,S,P,rho,drho_dT,drho_dS,drho_dT_dT, &
 ...
 dKdT = 148.4206D0 - T*(4.65421D0 - T*(4.081431D-2 - T*2.0621152D-4));
 
 ! calculate the 2nd derivative wrt T of the secant bulk modulus of pure water
 dKdTdT =  - (4.65421D0 - T*(8.162862D-2 - T*6.1863456D-4));
 
 ! calculate the bulk modulus at one atmosphere
 K = K +S*(54.6746D0 - T*(.603459D0 - T*(1.09987D-2 - T*6.1670D-5))) &
       +S*dum*(7.944D-2 + T*(1.6483D-2 - T*5.3009D-4));
 
 ! calculate derivative wrt T of the bulk modulus at one atmosphere
 dKdT = dKdT +S*(-.603459D0 + T*(2.19974D-2 - T*1.8501D-4)) &
       +S*dum*(1.6483D-2 - 1.06018D-3*T);

 ! calculate 2nd derivative wrt T of the bulk modulus at one atmosphere
 IF (PRESENT(drho_dT_dT))  &
     dKdTdT = dKdTdT +S*(2.19974D-2 - T*3.7002D-4 - dum*1.06018D-3);
 
 ! calculate the derivative wrt S of the bulk modulus at one atmosphere
 dKdS = 54.6746D0 - T*(.603459D0 - T*(1.09987D-2 - T*6.1670D-5)) &
       +dum*(1.1916D-1 + T*(2.47245D-2 - T*7.95135D-4));

 ! calculate the cross derivative wrt S and T 
 ! of the bulk modulus at one atmosphere
 IF (PRESENT(drho_dT_dS)) dKdSdT = - (.603459D0 - T*(2.19974D-2- T*1.8501D-4)) &
                                   + dum*(2.47245D-2 - T*1.59027D-3);
 
 ! correct the bulk modulus for pressure p
 K = K + P*(  (3.239908D0 + T*(1.43713D-3 + T*(1.16092D-4             &
                - T*5.77905D-7)))                                   &
            + S*(2.2838D-3 - T*(1.0981D-5 + T*1.6078D-6))           &
            + 1.91075D-4*S*dum                                      &
            + P*(   (8.50935D-5 - T*(6.12293D-6 - T*5.2787D-8))     &
                    +S*(-9.9348D-7 + T*(2.0816D-8 + T*9.1697D-10)) ));
 
 ! correct the derivative wrt T of the bulk modulus for pressure p
 dKdT = dKdT + P*(  (1.43713D-3 + T*(2.32184D-4 - T*1.733715D-6))   &
                  + S*(- 1.0981D-5 - 3.2156D-6*T)                   &
                  + P*(  (- 6.12293D-6 + 1.05574D-7*T)              &
                        + S*(2.0816D-8 + 1.83394D-9*T)   ));

 ! correct the 2nd derivative wrt T of the bulk modulus for pressure p
 IF (PRESENT(drho_dT_dT)) dKdTdT = dKdTdT + P*(  (2.32184D-4 - T*3.46743D-6)  &
                                                 - S*3.2156D-6                &
                                                 + P*(   1.05574D-7           &
                                                       + S*1.83394D-9   ));
 
 ! correct the derivative wrt S of the bulk modulus for pressure p
 dKdS = dKdS + P*(  (2.2838D-3 - T*(1.0981D-5 + T*1.6078D-6))       &
                   + 2.866125D-4*dum                                &
                   + P*(-9.9348D-7 + T*(2.0816D-8 + T*9.1697D-10))  );
 
 ! correct the cross derivative wrt S and T of the bulk modulus for pressure p
 IF (PRESENT(drho_dT_dS)) dKdSdT = dKdSdT - P*(  1.0981D-5 + T*3.2156D-6  &
                                                - P*(2.0816D-8 + T*1.83394D-9));
 ...
 dum=1.D0/(K-P)
 rho = (K*rho+1.D3*P)*dum;
 drho_dT = (K*drho_dT + (r - rho)*dKdT)*dum ;
 drho_dS = (K*drho_dS + (r - rho)*dKdS)*dum ;

 IF (PRESENT(drho_dT_dT)) drho_dT_dT=dum*(r*dKdTdT+2.D0*rdT*dKdT+K*drho_dT_dT  &
                                           +rho*(2.D0*dum*dKdT*dKdT-dKdTdT)    &
                                            -2.D0*dum*(r*dKdT+K*rdT)*dKdT);               
...
FUNCTION potential_temperature_T(T,S,P)
 ...
 potential_temperature_T = 1.D0 +                                  &
             P*( -8.3198D-5 + T*(1.0813D-6 - T*1.20822D-8)       &
                +(S-35.D0)*2.9778D-7                               &
                +P*( (3.1628D-8 - T*4.3974D-10) - P*5.0484D-12 ) );
...
FUNCTION potential_temperature_TT(T,P)
 ...
 potential_temperature_TT = P*( 1.0813D-6 - T*2.41644D-8 - P*4.3974D-10 );
...
FUNCTION potential_temperature_TS(P)
 ...
 REAL(8) :: P,potential_temperature_TS
  potential_temperature_TS = P*2.9778D-7 
...
! K=p/K
! densityTSP = 1000.*K/(1.-K) + densityTSP/(1.D0-K) 
  densityTSP = (1.D+3*P+densityTSP*K)/(K-P)
